...
class CompileDataset:
    ...
    def run(self, source: DatasetSource, output_path: Path, schema_index: int | None = None) -> tuple[Path, list[str]]:
        ...
        if len(schemas) > 1 and schema_index is None:
            labels = [f"Schema {index} ({schema.occurrence_count} files)" for index, schema in enumerate(schemas, start=1)]
            raise ValueError("Multiple schemas detected. Re-run with --schema. Available: " + ", ".join(labels))
        ...
        selected_files = [record.path for record in build_result.files if record.schema_hash == selected_schema.hash]
        output_path.mkdir(parents=True, exist_ok=True)
        written = write_parquet_dataset((_read_dataframe(path) for path in selected_files), output_path)
        ...
        skipped = [record.relative_path.as_posix() for record in build_result.files if record.schema_hash != selected_schema.hash]
