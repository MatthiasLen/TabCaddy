    ...
    def run(self, source: DatasetSource, output_path: Path) -> Path:
        ...
        lines.extend(
            [
                ...
                "",
                "    # Example: standardize a numeric column to zero mean and unit variance",
                "    # if 'quantity' in df.columns:",
                "    #     mean_value = df.select(pl.col('quantity').cast(pl.Float64).mean()).item()",
                "    #     std_value = df.select(pl.col('quantity').cast(pl.Float64).std()).item()",
                "    #     if std_value:",
                "    #         df = df.with_columns(((pl.col('quantity').cast(pl.Float64) - mean_value) / std_value).alias('quantity'))",
                "",
