...
def test_diff_reports_changes(tmp_path: Path) -> None:
    ...
    pl.DataFrame({"id": [1, 2, 3], "value": [10.0, 20.0, 40.0]}).write_csv(right / "data.csv")

    report = DiffDatasets(GenerateAnalysis()).run(resolve_source(left), resolve_source(right), DiffLevel.FULL)
    assert any("Row Count" in change or "rows" in change.lower() for change in report.metadata_changes)
